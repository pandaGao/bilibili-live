...
var _5_0_1_safeBuffer = createCommonjsModule(function (module, exports) {
...
const Buffer = _5_0_1_safeBuffer.Buffer;
...
const Buffer$1 = _5_0_1_safeBuffer.Buffer;
...
var Extensions = createCommonjsModule(function (module) {
'use strict';
...
const Buffer$2 = _5_0_1_safeBuffer.Buffer;
...
const Buffer$3 = _5_0_1_safeBuffer.Buffer;
...
const Buffer$4 = _5_0_1_safeBuffer.Buffer;
...
class WebSocket$1 extends EventEmitter {
  ...
  setSocket (socket, head) {
    ...
    this._ultron = new _1_1_0_ultron(socket);
    this._socket = socket;
...
const Buffer$5 = _5_0_1_safeBuffer.Buffer;
...
class WebSocketServer extends EventEmitter {
  ...
  constructor (options, callback) {
    ...
    if (this._server) {
      this._ultron = new _1_1_0_ultron(this._server);
...
class DanmakuService extends EventEmitter {
  ...
  constructor(config = {}) {
    ...
    this._checkErrorService =  lodash.debounce(() => {
  ...
  connect() {
    if (this.useWebsocket) {
      if (this.useWSS) {
        this._socket = new _3_0_0_ws(`${WSSDMPROTOCOL}://${WSDMSERVER}:${WSSDMPORT}/${WSDMPATH}`);
      } else {
        this._socket = new _3_0_0_ws(`${WSDMPROTOCOL}://${WSDMSERVER}:${WSDMPORT}/${WSDMPATH}`);
      }
    } else {
  ...
  bundleGift(msg) {
    ...
    if (this._giftBundleMap.has(key)) {
      ...
      let giftEvent = {
        msg: lodash.merge({}, msg),
        event: lodash.debounce(() => {
...
class InfoService extends EventEmitter {
  ...
  fetchInfo() {
    ...
    this._api.getRoomInfo().then(res => {
      if (ts < this._lastUpdate) return
      this._lastUpdate = ts;
      if (!lodash.isEqual(res, this._info)) {
        this._info = res;
...
class UserService extends EventEmitter {
  ...
  getUserInfo() {
    this._api.getUserInfo().then(res => {
      if (res) {
        if (!lodash.isEqual(res.user, this.info)) {
          this.info = res.user;
          this.emit('info.user', this.info);
        }
        if (!lodash.isEqual(res.room, this.room)) {
          this.room = res.room;
